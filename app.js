...
function Display() {
    ...
    if (status == "PASS") {
        ...
        display.innerHTML += "<h3 style='color: red;'> Candidate fail in Subject :- </h3> ";

        for (let c = 0; c < subject.length; c++) {
            
            if(subject[c].value < 40 ){
                display.innerHTML += ` <div class='print '> <p style='color: red;'> ${c+1} </p>  </div> `;
            }
        }

    }

    display.innerHTML += " <br> <div class='print '> <p> Subject </p> <p> Marks </p> </div>  ";
...
function validate() {
    ...
    for (let c = 0; c < subject.length; c++) {
        if (subject[c].value > 100 || subject[c].value < 0 || subject[c].value == "" ) {
            // console.log("wrong values ");
            subject[c].classList.add("error");
            subject[c].classList.remove("clear");
        }
        else {
            // console.log(" yes it is good to go  ");
            subject[c].classList.add("clear");
            subject[c].classList.remove("error");
        }
    }

    let clear = document.querySelectorAll(".clear");

    if(clear.length == subject.length  && subject.length !=0 ){
        Calculate();
        Display();
    }
